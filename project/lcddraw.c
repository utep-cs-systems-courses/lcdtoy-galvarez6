 ...
 */
void drawChar6x9(u_char rcol, u_char rrow, char c, 
     u_int fgColorBGR, u_int bgColorBGR) 
{
  ...
  lcd_setArea(rcol, rrow, rcol + 5, rrow + 9); /* relative to requested col/row */
  while (row < 10) {
    while (col < 7) {
      u_int colorBGR = (font_6x9[oc][col] & bit) ? fgColorBGR : bgColorBGR;
      lcd_writeColor(colorBGR);
      col++;
    }
    col = 0;
 ...
 */
void drawString6x9(u_char col, u_char row, char *string,
		u_int fgColorBGR, u_int bgColorBGR)
{
  u_char cols = col;
  while (*string) {
    drawChar6x9(cols, row, *string++, fgColorBGR, bgColorBGR);
    cols += 7;
  }
}
